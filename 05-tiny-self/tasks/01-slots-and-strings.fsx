...
let rec lookup (msg: string) (obj: Objekt) : list<Slot> =
    // Implement message lookup (as documented in the Self handbook)
...
// Scroll down, create the "Hello world" string object and
// visualize it to see what the representation looks like!

let lookupSlotValue (msg: string) (obj: Objekt) : Objekt =
    // Find the slot named 'n' in the object 'o' and return its contents
...
let getStringValue (obj: Objekt) : string =
    // Get the value of 'value' slot using 'lookupSlotValue'
    // This should be an object that has 'Special' set to 'Some str'
    // Return the string value!
    let sObj = lookupSlotValue "value" obj
...
// Cheshire cat has a name ("Cheshire Cat") and is
