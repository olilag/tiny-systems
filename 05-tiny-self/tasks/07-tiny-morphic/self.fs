...
stringPrototype.Slots <- makeSlot "render" (makeSelfMethod exprSelf) :: stringPrototype.Slots
...
let cat =
    makeObject
        [ makeSlot "sound" (makeString "Meow")
          makeSlot "say" (makeSelfMethod (exprSelf |> exprSend "sound" [] |> exprSend "print" []))
          makeSlot
              "render"
              (makeSelfMethod (
                  exprNew
                      [ "tag", exprString "div"
                        ...
                        exprNew
                            [ "1",
                              ...
                              exprNew
                                  [ "tag", exprString "img"
                                    "attrs", exprNew [ "src", exprSelf |> exprSend "image" [] ] ]
                              // Modify the rendering code for cats!
...
let animals =
    makeObject [ makeSlot "larry" larry; makeSlot "cheshire" cheshire; makeSlot "mog" mog ]


// Another demo - just a simple TinySelf hello world object
let greeterPrototype =
    makeObject [ makeSlot "greet" (makeSelfMethod (exprSelf |> exprSend "greeting" [] |> exprSend "print" [])) ]
...
let demo = makeObject [ makeSlot "animals" animals; makeSlot "greeter" greeter ]
